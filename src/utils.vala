...
public const string DEFAULT_ALARM_SOUND = "/usr/share/sounds/freedesktop/stereo/alarm-clock-elapsed.oga";
...
public class Bell : Object {
    ...
    private GLib.File sound_file () {
        var custom = settings.get_string ("alarm-sound-file");
        if (custom != null && ((string) custom).length > 0) {
            var custom_file = GLib.File.new_for_path ((string) custom);
            if (custom_file.query_exists ()) {
                return custom_file;
            }
            warning ("Custom alarm sound not found: %s", (string) custom);
        }

        return file;
    }

    private static string? pactl (string args) {
        try {
            string output;
            int status;
            Process.spawn_command_line_sync ("pactl " + args, out output, null, out status);
            return output;
        } catch (Error e) {
            warning ("pactl %s failed: %s", args, e.message);
            return null;
        }
    }

    private static string? default_sink () {
        var result_output = pactl ("get-default-sink");
        return result_output != null ? ((!) result_output).strip () : null;
    }

    private static SinkInfo[] sinks () {
        SinkInfo[] result = {};
        var result_output = pactl ("list sinks");
        if (result_output == null) {
            return result;
        }

        SinkInfo? current = null;
        foreach (var raw in ((!) result_output).split ("\n")) {
            var line = raw.strip ();
            if (line.has_prefix ("Sink #")) {
                if (current != null) {
                    result += current;
                }
                current = SinkInfo ();
                ...
                if (line.has_prefix ("Name:")) {
                    current.name = line.substring (5).strip ();
                } else if (line.has_prefix ("Description:")) {
                    current.description = line.substring (12).strip ();
                } else if (line.has_prefix ("State:")) {
                    current.state = line.substring (6).strip ();
                }
            }
        }
        if (current != null) {
        ...
        for (int i = 0; i < result.length; i++) {
            var name = result[i].name.down ();
            var description = result[i].description.down ();
            result[i].bluetooth = name.contains ("bluez") ||
                                  description.contains ("bluetooth") ||
                                  description.contains ("echo");
            result[i].hdmi = name.contains ("hdmi") || description.contains ("hdmi");
        }

        return result;
    }

    private string? choose_sink () {
        var available = sinks ();
        ...
        if (preference == "bluetooth") {
            foreach (var sink in available) {
                if (sink.bluetooth && sink.state != "UNCONNECTED") {
                    return sink.name;
                }
            }
        } else if (preference == "hdmi") {
            foreach (var sink in available) {
                if (sink.hdmi) {
                    return sink.name;
                }
            ...
            // auto: Bluetooth when connected, otherwise HDMI
            foreach (var sink in available) {
                if (sink.bluetooth && sink.state != "UNCONNECTED") {
                    return sink.name;
                }
    ...
    private void ensure_audio_output () {
        var target = choose_sink ();
        if (target == null) {
            return;
        }

        var current = default_sink ();
        if (current != target) {
            pactl ("set-default-sink %s".printf (target));
        }

        var volume = settings.get_double ("alarm-output-volume");
        if (volume < 0.0) {
            volume = 0.0;
        } else if (volume > 1.0) {
            volume = 1.0;
        ...
        pactl ("set-sink-mute %s 0".printf (target));
        pactl ("set-sink-volume %s %d%%".printf (target, (int) (volume * 100.0)));
    }

    private void ring_real (bool repeat) {
        // Play at the configured volume on the right device:
        // Bluetooth (Echo) when connected, otherwise HDMI
        ensure_audio_output ();

        media_file = Gtk.MediaFile.for_file (sound_file ());
    ...
    public static void play_test () {
        if (test_bell != null) {
            test_bell.stop ();
        }
        test_bell = new Bell (GLib.File.new_for_path (DEFAULT_ALARM_SOUND));
        test_bell.ring_once ();
    }
